...
4. **Swap biljartv2 over to consume the package.** This is the real proof step: not a demo of the extracted library, but the production system running on it. If this migration is painful, the extraction boundary was wrong and step 2 needs revisiting.
   ...
   - **(4b, in progress)** Replace the existing import code path with calls into the package, stage by stage, keeping the current UI/behavior unchanged.
     - Diff (league fixtures): done — `FixturesImportDiffService`'s league branch classifies via `StageGate\Classifier` + `LeagueFixturesImportDefinition`'s field groups instead of a locally duplicated field-list copy.
     - Diff (cup fixtures): done — `CupFixturesImportDefinition`/`ExistingRowsProvider`/`PublishWriter` added, mirroring the qualifier_type/qualifier_ref split/join logic. `classifyUpdatedRow()` (the old hardcoded per-type field lists) is gone entirely; both fixture types now flow through one `classifyUpdatedRowViaStageGate()`. All 320 existing BiljartV2 tests pass unchanged throughout both swaps, including exact count assertions — genuine behind-the-scenes swaps, not behavior changes.
     - Proof: intentionally *not* being swapped — its real validation (team alias resolution, season-mismatch checks) is bespoke business logic the generic `Proof::analyze()` can't replicate; swapping it would be pure risk for no gain.
     - Approve: doesn't get its own swap either — production's `FixturesImportApprovalService` gates on whole-batch validation errors only, not per-row overwrite-risk acknowledgment like the core's `Approve::approve()` supports. This folds into the Publish swap instead (`--allow-result-overwrite` maps to "treat all overwrite-risk rows as approved" vs. "approve none").
     - Still to do: the Publish swap itself (`FixturesImportPublicationService` → `Publish::plan()` + `PublishExecutor` + the League/Cup `PublishWriter`s) — the highest-stakes remaining piece, since it's the actual write path.

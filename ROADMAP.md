...
4. **Swap biljartv2 over to consume the package.** This is the real proof step: not a demo of the extracted library, but the production system running on it. If this migration is painful, the extraction boundary was wrong and step 2 needs revisiting.
   ...
   - **(4b, in progress)** Replace the existing import code path with calls into the package, stage by stage, keeping the current UI/behavior unchanged.
     - Diff (league fixtures): done — `FixturesImportDiffService`'s league branch now classifies via `StageGate\Classifier` + `LeagueFixturesImportDefinition`'s field groups instead of a locally duplicated field-list copy. Cup fixtures untouched, still on the original path, until a `CupFixturesImportDefinition` exists. All 320 existing BiljartV2 tests pass unchanged, including exact count assertions — genuine swap, not a behavior change.
     ...
     - Still to do: cup fixtures' `ImportDefinition`, and the Publish swap (`FixturesImportPublicationService` → `Publish::plan()` + `PublishExecutor` + `LeagueFixturesPublishWriter`).
